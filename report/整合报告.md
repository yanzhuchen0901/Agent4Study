...
> 本文档侧重评审可见的合并决策摘要、category 分布和前后对比。  
> 技术报告详情见 `report/competition_report.md`。
...
| LLM 合并决策数 | 3 |
| 估算去重率 | 13.04% |
| 合并来源教材 | demo_基础算法 + 数据结构 |

### 合并案例

| 决策 ID | 涉及节点 | LLM 建议 | 理由 |
|---------|---------|---------|------|
| merge_001 | 排序算法 / 排序 | merge | 语义高度相似，建议合并 |
| merge_002 | 深度优先搜索 / DFS | merge | 同一概念的不同表述 |
| merge_003 | 搜索 / 搜索算法 | merge | 语义高度相似，建议合并 |
