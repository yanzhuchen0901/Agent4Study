...
```json
{
  "id": "book_ch01_node_001",
  "name": "栈",
  "aliases": ["stack"],
  "definition": "后进先出的线性结构",
  "category": "核心概念",
  "importance": "high",
  "original_text": "栈是一种后进先出（LIFO）的线性表"
}
```
...
仅应用人工批准的 `merge` 决策；`keep` 和 `remove` 只记录决策，不删除节点。合并决策可包含 `best_node_id`，用于说明哪个候选版本作为主节点。
