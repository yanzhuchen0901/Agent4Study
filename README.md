...
- **三层知识层级**：教材层 → 章节层 → 知识点层，支持钻取和面包屑导航。
...
3. 点击”构建层级”生成三层知识结构（教材层 / 章节层 / 知识点层）。
4. 使用顶部 `教材层 | 章节层 | 知识点层` 切换当前层级视图。
5. 在教材层点击教材节点 → 自动钻取到该教材的章节层。
6. 在章节层点击章节节点 → 自动钻取到该章节的知识点层。
7. 通过面包屑导航（全部教材 > 某教材 > 某章节）返回任意上级层级。
8. 进入”RAG 问答”建立索引并提问。
9. 进入”知识合并”生成合并候选并人工确认。
10. 进入”Agent”提出复杂问题，查看实时工作流。
11. 进入”报告”查看竞赛报告 Markdown。

### 三层知识层级说明

| 层级 | 说明 | 节点形状 | 关系类型 |
|------|------|---------|---------|
| 教材层 | 每本教材作为一个节点 | 矩形（蓝色） | overlap（概念重叠）、prerequisite |
| 章节层 | 每章作为一个节点 | 圆角矩形（琥珀色） | contains（教材→章节）、overlap（跨书章节） |
| 知识点层 | LLM 抽取的核心概念 | 椭圆（按类别着色） | prerequisite/parallel/contains/applies_to/overlap |

教材间和跨书章节间的关系基于概念关键词 Jaccard 相似度自动计算（阈值 ≥ 0.06）。
